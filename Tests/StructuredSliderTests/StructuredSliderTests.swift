...
final class StructuredSliderTests: XCTestCase {
    func testElementWidth() {
        let steps = Array(repeating: StructuredSlider.Step(value: "x", shortValue: "x"), count: 6)
    ...
    func testProgress() {
        let steps = Array(repeating: StructuredSlider.Step(value: "x", shortValue: "x"), count: 5)
